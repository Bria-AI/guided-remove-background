...
def run_one(case: dict, mode: str) -> dict:
    ...
    if not image_path.exists():
        return {
            ...
            "mode": mode, "error": f"Image not found: {image_path}", "elapsed_s": 0,
        }
    ...
    try:
        ...
        return {
            ...
            "prompts": prompts,
            "mode": mode, "error": None, "elapsed_s": result.elapsed_s,
...
def main() -> None:
    ...
    log.info("Running %d case(s) x %d mode(s) = %d total", len(cases), len(modes), len(cases) * len(modes))

    all_results: list[dict] = []
    t0 = time.monotonic()
    jobs = [(c, m) for c in cases for m in modes]
    ...
    def _flush_meta(done: bool = False) -> None:
        ...
        meta = {
            ...
            "results": sorted(all_results, key=lambda r: (r["mode"], r["image"], r["foreground"])),
        }
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as pool:
        futures = {pool.submit(run_one, case, mode): (case, mode) for case, mode in jobs}
